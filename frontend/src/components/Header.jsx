...
import { Heart, Menu, PlusCircle, Store, UserPlus, UserRound, X } from 'lucide-react'
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur">
      <div className="mx-auto flex h-18 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        <Link to="/" className="flex shrink-0 items-center gap-2.5" aria-label="Chợ Đồ Cũ - Trang chủ">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Điều hướng chính">
          ...
          <button
            type="button"
            onClick={onShowSaved}
            className={`rounded-lg px-3 py-2 text-sm font-medium transition ${savedOnly ? 'bg-emerald-50 text-emerald-700' : 'text-slate-600 hover:bg-emerald-50 hover:text-emerald-700'}`}
          >
            Tin đã lưu
          </button>
        </nav>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          ...
          >
            <Heart size={19} className={savedCount ? 'fill-emerald-600 text-emerald-600' : ''} />
            {savedCount > 0 && (
          ...
          <button
            type="button"
            onClick={() => onUnavailable('Đăng nhập')}
      ...
      {menuOpen && (
        <div className="border-t border-slate-100 bg-white px-4 py-4 shadow-lg lg:hidden">
          <nav className="mx-auto grid max-w-7xl gap-1" aria-label="Điều hướng di động">
            ...
            >
              Tin đã lưu
              <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-bold text-emerald-700">{savedCount}</span>
            </button>
          </nav>
          <div className="mx-auto mt-3 grid max-w-7xl grid-cols-2 gap-2 border-t border-slate-100 pt-3 sm:grid-cols-3">
            <button type="button" onClick={() => onUnavailable('Đăng tin')} className="rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-semibold text-white">
              Đăng tin
            </button>
            <button type="button" onClick={() => onUnavailable('Đăng nhập')} className="rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-semibold text-slate-700">
              Đăng nhập
            </button>
            <button type="button" onClick={() => onUnavailable('Đăng ký')} className="col-span-2 rounded-xl border border-emerald-600 px-3 py-2.5 text-sm font-semibold text-emerald-700 sm:col-span-1">
              Đăng ký
            </button>
          </div>
        </div>
